...
async def build_app(*, llm, repo, slug: str, name: str, goal: str,
    ...
    try:
        ...
        # Add double-click launchers (run.bat / run.sh) so the operator can run the
        # build without opening it in IDLE. Best-effort; only when there's a Python
        # entry point to launch.
        entry = _entry_point(build_dir)
        if entry:
            try:
                for f in write_launchers(build_dir, entry):
                    if f not in files:
                        files.append(f)
            except Exception:
                logger.exception("write_launchers failed for %s", slug)
...
def write_launchers(build_dir: Path, entry: Path) -> list[str]:
    """Drop double-click launchers at the build root so the operator never has to
    open the project in IDLE: run.bat (Windows) + run.sh (Unix). They cd to the
    entry point's folder, run it with the system Python, and keep the window open.
    Returns the launcher filenames written (relative to build_dir)."""
    rel = entry.relative_to(build_dir)
    sub = rel.parent
    win_sub = "" if str(sub) == "." else "\\" + str(sub).replace("/", "\\")
    posix_sub = "" if str(sub) == "." else "/" + str(sub)
    name = entry.name

    bat = (
        "@echo off\r\n"
        "rem Auto-generated by R.A.M.B.O — double-click to run this build.\r\n"
        f'cd /d "%~dp0{win_sub}"\r\n'
        f'where py >nul 2>nul && (py "{name}" %*) || (python "{name}" %*)\r\n'
        "echo.\r\n"
        "pause\r\n"
    )
    (build_dir / "run.bat").write_text(bat, encoding="utf-8", newline="")

    sh = (
        "#!/usr/bin/env bash\n"
        "# Auto-generated by R.A.M.B.O — run this build.\n"
        f'cd "$(dirname "$0"){posix_sub}" || exit 1\n'
        f'python3 "{name}" "$@" 2>/dev/null || python "{name}" "$@"\n'
    )
    sh_path = build_dir / "run.sh"
