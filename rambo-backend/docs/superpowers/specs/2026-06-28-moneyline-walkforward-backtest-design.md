...
- **Depth:** two snapshots per game — a morning line (the price actually bet) and a closing line (first pitch) — enabling real ROI *and* CLV.
...
`moneyline_model.evaluate_game_asof(repo, season, game, before_date)` mirrors `evaluate_game` but calls the `_asof` reads. It keeps the market-anchoring path (anchors to the historical de-vigged morning line for that date) — anchoring is honest here because we have the real past line.
...
- `brains/ev/walkforward.py` — `run(repo, start, end) -> dict`
  ...
  2. Anchor to D's de-vigged **morning** line; pick the side the model leans (model prob > anchored book prob).
  3. Emit a backtest record `{p, win, odds, close}`: `p` = model prob for the picked side, `win` = did that side win (from final score), `odds` = morning American price for that side, `close` = closing American price for that side.
  Feed the full record set to `backtest.evaluate()` → `{n, win_rate, roi, brier, log_loss, avg_clv, calibration}`.
...
odds_backfill.py ─historical snapshots─▶ odds_lines (morning + closing)
                                            │
walkforward.run(start,end)                  │
  └ per game-day D:                          │
      evaluate_game_asof(before_date=D) ◀─ mlb_repo._asof reads (game logs < D)
      anchor to morning de-vig line ◀────────┘
      pick side → record{p,win,odds,close}
...
- `test_walkforward.py` — leak guard (a feature as-of date D never reflects game D's result), record assembly (correct side/price/win mapping), coverage counters.
- `test_odds_backfill.py` — snapshot timestamp derivation (morning/closing offsets), per-day slate dedup, idempotent re-run.
...
- CLV uses the morning→closing move on the *same book selection rule*; it is a directional signal, not a guaranteed realizable number.
