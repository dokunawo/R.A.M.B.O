...
### Next action
Daniel was testing the UI after the response branches commit. Likely next requests: visual polish on the new branch panels, Gmail/Spotify integration, or testing the calendar/drive skills through the voice interface. Wait for direction.
...
| File | Purpose |
|------|---------|
| `SplashScreen.js` / `.css` | Main UI — Phase 1 (boot) + Phase 2 (console). ~1250 lines. The command input, agent roster, system params, and ResultBranch are all here. |
| `AgentPage.js` / `.css` | Per-agent detail pages. Left/right panels, response tree branches, quick-switch bar. AGENT_META object has all 10 agents' metadata. |
...
| `VoiceControls.jsx` | `usePageVoice()` hook (auto-starts mic, executes commands against backend), `VoiceControls` component (mic/volume buttons), `CommandLog` component (used on LearningLog and RoundTable, replaced by ResponseTree on AgentPage). |
...
| `north-star.md` | Daniel's business doctrine — armed security + photography, $10K target, operating rules. **Gitignored.** |
...
### Do not confuse
- `RamboOrb3D.jsx` / `RamboOrbShaders.js` — **legacy orb files**, not used. `CosmicOrb.jsx` + `CosmicOrbShaders.js` replaced them.
- `HudLayout.js` / `.css` — **legacy layout**, not used since Phase 2 redesign.
- `BrainFeed.js` — **legacy activity feed**, not used.
...
- **PowerShell commit messages** — parentheses and special characters in commit messages get parsed by PowerShell and break `git commit -m`. Always use Bash with heredoc for commit messages, never PowerShell with `@'...'@` for complex messages.
...
- **Edit tool indentation** — when editing files with the Edit tool, indentation must match exactly. Read the file first and use the exact whitespace. `replace_all` with wrong indentation fails silently.
...
- **Is the response tree branch layout visually right?** It was just committed — Daniel hasn't confirmed the look yet. He may want adjustments to positioning, sizing, or the connector style.
- **Does Daniel want the personality voice activated?** He needs to get an Anthropic API key. He said "skip this for now" but may circle back.
- **Should the north-star.md be editable through the R.A.M.B.O UI?** Currently it's a static file.
- **What's the desired behavior when backend is offline?** Currently skills fail silently with error messages. No offline mode or cached responses.
